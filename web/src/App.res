...
module State = {
  ...
  let reducer = (state, action: Action.t) => {
    ...
    | InputTaxId(taxId_) => {
        let taxId = taxId_->String.trim->String.replaceRegExp(/[^0-9]/g, "")
...
module ThisDate = {
  ...
  let toDate = (d: t) => Date.makeWithYMD(~day=d.day, ~month=d.month - 1, ~year=d.year)
}
...
let make = () => {
  ...
  <div className="h-full flex flex-col justify-start items-center gap-4 p-4 text-sm md:text-lg">
    ...
    >
      ...
      />
      <button
        className={"px-4 py-2 rounded-sm border-2 focus:outline-hidden font-semibold " ++
        inputClass}
        type_="button"
